...
class DatabaseConnector:
    ...
    def execute(self, query, params=()):
        """Выполнить произвольный SQL-запрос с параметром"""
    ...
    def fetch_data(self, query, params=None):
        try:
            # Создаем временный словарь с параметрами подключения
            connection_params = {
                'dbname': self.dbname,
                'user': self.user,
                'password': self.password,
    ...
    def insert_user(self, username, hashed_password):
        """Метод для вставки нового пользователя с хэшированным паролем."""
        query = """
    ...
    def fetch_services(self):
        """Возвращает список всех услуг из БД, включая возможные вариации количества и цен."""
        query = """
    ...
    def update_service(self, service_id, new_name, new_price):
        """Изменяет название и цену услуги."""
    ...
    def update_profile(self, first_name, last_name, phone, email, address):
        ...
        query = """
            ...
            WHERE id=%s;
        """
        self.execute(query, (first_name, last_name, phone, email, address, 1))  # Предполагается, что id=1
        self.conn.commit()
...
class LoginForm(QWidget):
    ...
    def check(self):
        ...
        if db_connector.connect():
            ...
            if result:
                stored_hash = result[0][0].tobytes()

                # Проверяем введённый пароль
                try:
                    if bcrypt.checkpw(password.encode('utf-8'), stored_hash):
                        QMessageBox.information(self, 'Уведомление', 'Вы успешно прошли авторизацию!')
                        if username == "admin":
                            self.window = AdminWindow()  # Открываем окно для администратора
                            self.window.show()
                        else:
                            self.window = MainWindow()  # Открываем окно для обычных пользователей
                            self.window.show()
                        self.close()  # Закрываем текущее окно (окно входа)
                    else:
...
class AdminWindow(QWidget):
    ...
    def delete_user(self):
        ...
        reply = QMessageBox.question(self, 'Подтверждение', 'Вы уверены, что хотите удалить выбранных пользователей?',
    ...
    def delete_service(self):
        ...
        reply = QMessageBox.question(self, 'Подтверждение', 'Вы уверены, что хотите удалить выбранные услуги?',
                                     QMessageBox.Yes | QMessageBox.No)
        if reply == QMessageBox.Yes:
            for row in sorted(selected_rows, reverse=True):
                service_id = self.services_table.item(row, 0).text()
...
class ProfileEditDialog(QDialog):
    def __init__(self, profile_data, database_connector, parent=None):
        super().__init__(parent)
        ...
        # Имя
        self.first_name_input = QLineEdit(profile_data.get("first_name", ""))
        layout.addRow("Имя:", self.first_name_input)

        # Фамилия
        self.last_name_input = QLineEdit(profile_data.get("last_name", ""))
        layout.addRow("Фамилия:", self.last_name_input)

        # Телефон
        self.phone_input = QLineEdit(profile_data.get("phone", ""))
        layout.addRow("Телефон:", self.phone_input)

        # Email
        self.email_input = QLineEdit(profile_data.get("email", ""))
        layout.addRow("Email:", self.email_input)

        # Адрес
        self.address_input = QLineEdit(profile_data.get("address", ""))
    ...
    def save_changes(self):
        ...
        self.database_connector.update_profile(first_name, last_name, phone, email, address)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Создаем экземпляр класса DatabaseConnector
        self.database_connector = DatabaseConnector(
            dbname="postgres",
            user="postgres",
            password="22222",
        ...
        # Новый горизонтальный макет для поиска
        ...
        # Поля ввода для даты начала и конца
    ...
    def create_services_tab(self):
        ...
        # Увеличили количество колонок до 6, одна из которых скрытая
        self.services_table = QTableWidget(0, 6)

        # Отключаем возможность редактирования ячеек в таблице
        self.services_table.setEditTriggers(QAbstractItemView.NoEditTriggers)

        self.services_table.setStyleSheet("QTableWidget {background-color: #FFFFFF; }")
        self.services_table.setFixedWidth(1100)
        self.services_table.setMinimumHeight(500)

        # Названия колонок (видимы только первые 5 колонок)
        headers = ["ID", "Время", "Наименование", "Количество", "Примечание", "Сумма"]
        self.services_table.setHorizontalHeaderLabels(headers)

        # Первый столбец (ID) делаем скрытым
        self.services_table.setColumnHidden(0, True)

        # Режим растяжения применяется только к видимым колонкам
        header = self.services_table.horizontalHeader()
        for i in range(1, header.count()):  # Начинаем с единицы, пропускаем первый (скрытый) столбец
            header.setSectionResizeMode(i, QHeaderView.Stretch)

        # Загружаем начальные данные из базы данных
        ...
        # Связываем кнопки с обработчиками событий
    ...
    def create_report_tab(self):
        ...
        form_layout.addRow("Имя:", QLineEdit("Карина"))
    ...
    def load_services_to_table(self):
        ...
        query = f"""
            SELECT id, time, name, amount, notation, summ 
            FROM services 
            WHERE service_date = '{formatted_today}'
            ORDER BY id ASC;
        """
        ...
        # Включаем скрытый столбец с ID
        self.services_table.setColumnCount(6)
        self.services_table.setColumnHidden(0, True)  # Скрываем первую колонку (идентификатор)

        # Правильно размещаем данные в соответствующих колонках
        for row_num, row_data in enumerate(data):
            # Расшифровка данных из строки результата
            record_id, time_val, name_val, amount_val, notation_val, summ_val = row_data

            # Устанавливаем данные явно в нужные колонки
            self.services_table.setItem(row_num, 0, QTableWidgetItem(str(record_id)))  # Скрытая колонка ID
            self.services_table.setItem(row_num, 1, QTableWidgetItem(str(time_val)))  # Вторая колонка - Время
            self.services_table.setItem(row_num, 2, QTableWidgetItem(name_val))  # Третья колонка - Наименование
            self.services_table.setItem(row_num, 3, QTableWidgetItem(str(amount_val)))  # Четвертая колонка - Количество
            self.services_table.setItem(row_num, 4, QTableWidgetItem(notation_val))  # Пятая колонка - Примечание
            self.services_table.setItem(row_num, 5, QTableWidgetItem(str(summ_val)))  # Шестая колонка - Сумма
    ...
    def create_profile_tab(self):
        ...
        form_layout = QFormLayout()
        form_layout.addRow("Имя:", QLabel("Карина"))
        form_layout.addRow("Фамилия:", QLabel("Иванова"))
        form_layout.addRow("Телефон:", QLabel("+7 (999) 123-45-67"))
        form_layout.addRow("Email:", QLabel("karina@example.com"))
        form_layout.addRow("Адрес:", QLabel("Москва, ул. Ленина, д. 1"))
    ...
    def open_edit_profile_dialog(self):
        ...
        # Пример данных пользователя (это нужно адаптировать согласно реальной структуре)
        profile_data = {
            "first_name": "Карина",
            "last_name": "Иванова",
        ...
        dialog = ProfileEditDialog(profile_data, self.database_connector, self)
        dialog.exec_()
    ...
    def export_report(self):
        ...
        with open(filename, 'w', newline='') as file:
            ...
            writer.writerow(["Время", "Наименование", "Количество", "Цена", "Тип оплаты", "Итоговая стоимость"])
            # Здесь должна быть логика для получения данных из таблицы services_tree
            # Пример:
            # for child in services_tree.get_children():
            #     values = services_tree.item(child, 'values')
            #     writer.writerow(values)
        QMessageBox.information(self, "Информация", f"Отчет успешно экспортирован в файл {filename}.")
